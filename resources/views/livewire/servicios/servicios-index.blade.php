<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-purple-700"><i class="fa-solid fa-scissors mr-2"></i>Servicios</h1>
            <p class="text-gray-500 text-sm mt-1">Gestioná los servicios de la estética</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow p-4 mb-4">
        <input wire:model.live="search" type="text" placeholder="🔍 Buscar servicio..."
            ...
            <tbody>
                @forelse ($servicios as $servicio)
                    <tr class="border-t hover:bg-purple-50 transition">
                        <td class="p-4 font-medium text-gray-800">{{ $servicio->nombre }}</td>
                        <td class="p-4 text-gray-500">{{ $servicio->descripcion ?? '-' }}</td>
                        <td class="p-4 text-green-600 font-semibold">${{ number_format($servicio->precio, 2) }}</td>
                        <td class="p-4">
                            <span class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-medium">
                                <i class="fa-solid fa-clock mr-1"></i>{{ $servicio->duracion_minutos }} min
                            </span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-8 text-center text-gray-400">
                            <i class="fa-solid fa-scissors text-4xl mb-2 block"></i>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
                <h2 class="text-xl font-bold text-purple-700 mb-4">
                    ...
                    @if($servicioId) Editar Servicio @else Nuevo Servicio @endif
                </h2>
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div>
                        <label class="block text-sm text-gray-600 mb-1">Duración (min)</label>
                        <input wire:model="duracion_minutos" type="number"
                            class="border border-gray-200 rounded-lg w-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-400" />
