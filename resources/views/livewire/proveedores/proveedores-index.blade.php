<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-purple-700"><i class="fa-solid fa-truck mr-2"></i>Proveedores</h1>
            <p class="text-gray-500 text-sm mt-1">Gestioná los proveedores de productos</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow p-4 mb-4">
        <input wire:model.live="search" type="text" placeholder="🔍 Buscar proveedor..."
                ...
                @forelse ($proveedores as $proveedor)
                    <tr class="border-t hover:bg-purple-50 transition">
                        <td class="p-4 font-medium text-gray-800">
                            <i class="fa-solid fa-truck text-purple-400 mr-1"></i>{{ $proveedor->nombre }}
                        </td>
                        <td class="p-4 text-gray-600">{{ $proveedor->telefono ?? '-' }}</td>
                        <td class="p-4 text-gray-600">{{ $proveedor->email ?? '-' }}</td>
                        <td class="p-4 text-gray-600">{{ $proveedor->direccion ?? '-' }}</td>
                        <td class="p-4 flex gap-2">
                            <button wire:click="editar({{ $proveedor->id }})"
                            ...
                            <button wire:click="eliminar({{ $proveedor->id }})"
                                wire:confirm="Seguro que queres eliminar este proveedor?"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="p-8 text-center text-gray-400">
                            <i class="fa-solid fa-truck text-4xl mb-2 block"></i>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
                <h2 class="text-xl font-bold text-purple-700 mb-4">
                    ...
                    @if($proveedorId) Editar Proveedor @else Nuevo Proveedor @endif
                </h2>
